...
describe('Cloud Run Trading Cockpit API', () => {
  ...
  it('redacts the spreadsheet ID from production error responses', async () => {
    ...
    expect(errorSpy.mock.calls[0]).toEqual([
      '[trading-cockpit-api] request failed',
  ...
  it('loads production startup configuration from Cloud Run compatible environment variables', () => {
    expect(
      loadConfig({
        ...
        TRADING_COCKPIT_WEB_DIST_DIR: '/app/apps/web/dist',
        PORT: '9090'
      })
    ).toEqual({
      spreadsheetId: 'spreadsheet-id',
      port: 9090,
      staticAssetsPath: '/app/apps/web/dist',
      auth: {
        googleClientId: 'google-client-id',
  ...
  it('creates a Trade Plan through core and writes Trade Plans plus Watchlist status', async () => {
  ...
  it('records a capital transaction through core and appends to Capital Ledger', async () => {
