...
for (const filePath of coreFiles) {
  ...
  for (const providerName of forbiddenProviderNames) {
    if (
      ...
      violations.push(`${repositoryPath}: external provider name ${providerName} leaked into core`);
    }
  }
  ...
  for (const match of source.matchAll(
    ...
    if (forbiddenBackendCoreImports.some((forbidden) => specifier.includes(forbidden))) {
      violations.push(`${repositoryPath}: forbidden runtime/app dependency ${specifier}`);
    }
  }
}
...
for (const filePath of webFiles) {
  ...
  if (/from\s+['"][^'"]*backend\//.test(source)) {
    violations.push(`${repositoryPath}: web must not import sheets/api implementation`);
  }
...
if (violations.length > 0) {
  ...
  console.log(
    `Architecture check passed: ${sourceFiles.length} sheets, ${coreFiles.length} core, and ${webFiles.length} web modules, ` +
      'no core/port dependency on adapters, Apps Script globals, or external provider names; ' +
      'no core dependency on runtime infrastructure; no non-theme legacy global dependency.'
  );
}
