...
| Finviz.js          | Imports configured Finviz screeners and starts signal archiving                       | UrlFetch/Finviz; writes screener sheets; calls Strategy, SignalsHistory, Theme | None                                                                                        | KEEP_TEMPORARILY | External integration requires its own adapter migration                              |
| FinvizAuth.js      | Manages Finviz token in UserProperties and validates authentication                   | Prompts/alerts; UrlFetch; UserProperties                                       | None                                                                                        | KEEP_TEMPORARILY | Active authentication workflow; migration risk is external behavior                  |
...
| MomentumRanking.js | Builds the current momentum ranking from screener data                                | Reads screener/config; writes Momentum Ranking; calls scoring, Strategy, Theme | `MomentumScore.ts` only characterizes score parity                                          | KEEP_TEMPORARILY | Strategy/ranking workflow needs a future slice                                       |
| MomentumScore.js   | Five pure scoring functions used by MomentumRanking                                   | No direct Sheets access                                                        | Partial parity in `MomentumScore.ts`                                                        | KEEP_TEMPORARILY | Active ranking still calls the global JS functions                                   |
...
| SignalsHistory.js  | Archives screener observations with duplicate keys                                    | Reads screener/history; writes Signals History; calls Theme                    | None                                                                                        | KEEP_TEMPORARILY | Active Finviz ingestion history workflow                                             |
...
| Workflow.js        | Updates Watchlist status and provides manual status reconciliation                    | Reads Positions/Trade Plans; writes Watchlist statuses                         | TS Watchlist repository calls `updateWatchlistStatus`                                       | KEEP_TEMPORARILY | Status update is still an outbound adapter dependency; reconciliation remains legacy |
...
```text
Finviz -> Config, Strategy/getStrategy, SignalsHistory/archiveSignals, Theme/themeSimpleSheet
MomentumRanking -> Config, Strategy/getStrategy, MomentumScore/*, Theme/themeRanking
...
SignalsHistory -> Config, Utils, Theme/themeTechnicalSheet
Watchlist/TradePlan/Position/Journal -> Config, Utils, Theme (sheet adapter helpers only)
Workflow/updateWatchlistStatus -> Config, Utils; called by the TS Watchlist repository
...
`setupTradingAccounts`, `recordInitialFunding`, `recordDeposit`, and `recordWithdrawal`. `onOpen` is
also generated and delegates to the bundled TypeScript menu adapter.
...
and Watchlist status persistence. These declarations are located at the outbound adapter boundary;
none leak into Core or ports.

## Global surface and deployment

Before consolidation, deployment contained 20 manually maintained root JavaScript files plus one
generated bundle. It exposed 152 global functions and 20 menu targets. The menu migration removes
one manual file; deleting five superseded functions reduces the intended global surface while the
generated `onOpen` preserves the trigger.
...
1. Momentum Ranking and Momentum Score, including strategy/config boundaries.
2. Finviz authentication, ingestion, and Signals History adapters.
3. Analytics and Dashboard account-aware query models.
4. TypeScript Google Sheets schema/formula/formatting helpers for Watchlist, Trade Plan, Position,
   Journal, Strategy, Workflow, Setup, Theme, and scoped Utils.
5. Documentation sheet generation after its runtime value is reviewed.
