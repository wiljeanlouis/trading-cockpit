...
The original schema had 25 columns. Multi-account migration appends Account ID as column Z, leaving
all historical column letters and formulas unchanged. All columns are physically editable because
the sheet is not protected. “System” below describes
...
| Z      | Account ID       | Open Position command    | String          | Account attribution       | Uppercase value    | System/manual migration | Required for new Positions       |
...
Existing rows receive only the new header and retain a blank Account ID. No account is inferred.
Users must assign a configured Account ID explicitly before those Positions can be closed or
reconciled through the migrated workflows.
