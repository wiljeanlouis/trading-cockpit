...
describe('Momentum Ranking DATA sheet contract', () => {
  ...
  it('migrates historical row-5 ranking records to row 1 and is idempotent', () => {
    const legacyRow = rowFor(MOMENTUM_RANKING_HEADERS, {
      Rank: 1,
      'Strategy ID': 'MOMENTUM_BREAKOUT',
      Strategy: 'Momentum Breakout',
      'Strategy Version': 'V1',
    ...
    expect(migrateMomentumRankingSheet(sheet as never)).toEqual({
      status: 'MIGRATED',
      preservedRecords: 1,
      message: 'Momentum Ranking normalisé. 1 record(s) préservé(s).'
    });
    expect(sheet.values[0]).toEqual([...MOMENTUM_RANKING_HEADERS]);
    expect(sheet.values[1]).toEqual(legacyRow);
    expect(migrateMomentumRankingSheet(sheet as never)).toEqual({
  ...
  it('refuses unexpected layouts without clearing data', () => {
    const sheet = new MockSheet([['Important manual note'], ['Do not erase']]);
    ...
    expect(sheet.values).toEqual([['Important manual note'], ['Do not erase']]);
  });
});
